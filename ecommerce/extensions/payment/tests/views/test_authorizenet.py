...
class AuthorizeNetNotificationViewTests(PaymentEventsMixin, TestCase):
    ...
    def get_notification_response(
            self, responseCode, event_type=NOTIFICATION_TYPE_AUTH_CAPTURE, transaction_id=None):
        """
    ...
    @patch('ecommerce.extensions.payment.views.authorizenet.logger', autospec=True)
    @patch(
        'ecommerce.extensions.payment.views.authorizenet.AuthorizeNetNotificationView.payment_processor',
        autospec=True
    )
