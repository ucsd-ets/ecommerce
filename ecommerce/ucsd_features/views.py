...
class AssignVoucherView(APIView):
    ...
    def post(self, request):
        ...
        if remaining_vouchers_count == 0:
            logger.exception(
                'Vouchers count for course: %s is 0 therefore no more coupons will be assigned to any user', course_key)
        ...
        try:
            ...
            logger.info('Successfully sent an email to user: %s about assigned voucher', user_email)
...
class CourseCouponView(APIView):
    ...
    def post(self, request):
        ...
        if filtered_coupon_products:
            logger.info('%d coupon(s) found for course: %s', len(filtered_coupon_products), course_key)
            return JsonResponse({
                'found': True
            }, status=200)

        else:
            logger.info('No coupons found for course: %s', course_key)
            return JsonResponse({
                'found': False
            }, status=400)
